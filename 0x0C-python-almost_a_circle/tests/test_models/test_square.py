...
class TestSquare(unittest.TestCase):
    ...
    def test_206_str(self):
        ...
        self.assertEqual(Square.__str__
                         (self.square1), "[Square] (51) 0/0 - 5")
        self.assertEqual(Square.__str__
                         (self.square2), "[Square] (52) 0/0 - 7")
        self.assertEqual(Square.__str__
                         (self.square3), "[Square] (53) 2/0 - 2")
        self.assertEqual(Square.__str__
                         (self.square4), "[Square] (54) 1/2 - 3")

    def test_207_dict(self):
        """Checking dict"""
        dict1 = self.square1.to_dictionary()
        self.assertDictEqual(dict1, {'size': 5, 'id': 51, 'x': 0, 'y': 0})
        dict2 = self.square2.to_dictionary()
        self.assertDictEqual(dict2, {'size': 7, 'id': 52, 'x': 0, 'y': 0})
        dict3 = self.square3.to_dictionary()
        self.assertDictEqual(dict3, {'size': 2, 'id': 53, 'x': 2, 'y': 0})
        dict4 = self.square4.to_dictionary()
        self.assertDictEqual(dict4, {'size': 3, 'id': 54, 'x': 1, 'y': 2})
    ...
    def test_210_id(self):
        """Checking id"""
        self.assertEqual(self.square1.id, 51)
        self.assertEqual(self.square2.id, 52)
        self.assertEqual(self.square3.id, 53)
        self.assertEqual(self.square4.id, 54)
